...
export default function Ratings({ route, navigation }) {
  ...
  useEffect(() => {
    const checkReservation = async () => {
      try {
        ...
        const now = new Date();
        const reservationsRef = await firebase.firestore().collection('reserva')
        .where('id_auto', '==', carId)
      ...
      if (!reservationsRef.empty) {
        const now = new Date();
        const pastReservations = reservationsRef.docs.filter(doc => doc.data().fecha_fin.toDate() < now);
  ...
  const saveRating = async () => {
    try {
      if (!hasReserved) {
        Alert.alert('No se puede calificar', 'Debes haber reservado este auto anteriormente y la reserva debe haber finalizado.');
        return;
      }
